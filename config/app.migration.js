...
const { log, LogLevel, asyncForEach } = require(path.resolve(__dirname, 'helper'));
const devicePropertiesFile = path.resolve(__dirname, 'properties/devices.properties.json');
...
const fullTestForReleaseStrategies = require(path.resolve(__dirname, `strategies.release/ch.mobiliar.${projectId}`));
...
execute('idevice_id -l').then(async attachedUdids => {
    ...
    asyncForEach(fullTestForReleaseStrategies.strategies, (strategy, strategyIndex) => {
        if (!strategy.enabled) {
            return true;
        }

        const { debug, mode } = strategy;
        log(LogLevel.Log, `Starting Automation Strategy for: ${fullTestForReleaseStrategies.bundleId}`);
        ...
        let params;
        let signingIdentity = {
            xcodeOrgId: fullTestForReleaseStrategies.xcodeOrgId,
            xcodeSigningId: fullTestForReleaseStrategies.xcodeSigningId,
            bundleId: fullTestForReleaseStrategies.bundleId,
            appPath: fullTestForReleaseStrategies.appPath,
            simulatorAppPath: fullTestForReleaseStrategies.simulatorAppPath
        };
        ...
        switch (mode) {
            case 'SINGLE':
                ...
                log(LogLevel.Log, `Running automation test on ${singleDevice.deviceName} - iOS ${singleDevice.platformVersion} ...`);
                configFilePath = `tmp/device-${strategyIndex}.json`;
                params = ['./config/wdio.conf.js', '--deviceInfo', configFilePath];
                singleDevice = {
                    ...singleDevice,
                    ...strategy,
                    ...signingIdentity
            ...
            case 'MULTI':
                attachedDevices.forEach((attachedDevice, index) => {
                    log(LogLevel.Log, `Running automation test on ${attachedDevice.deviceName} - iOS ${attachedDevice.platformVersion} ...`);
        ...
        forEach(strategy.suites, (suite, suiteName) => {
            ...
            suites[suiteName] = map(suite.testCases, testCase =>
                path.join(
                    process.cwd(),
                    '/dist/apps/',
                    fullTestForReleaseStrategies.bundleId,
                    'specs',
                    suite.deviceType ? toLower(suite.deviceType) : 'common',
                    testCase
                )
            );
