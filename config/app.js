...
const strategies = require(path.resolve(__dirname, `strategies.dev/ch.mobiliar.${projectId}`));
...
execute('idevice_id -l').then(async attachedUdids => {
    ...
    asyncForEach(strategies.strategies, (strategy, strategyIndex) => {
        if (!strategy.enabled) {
            return true;
        }

        const { debug, mode } = strategy;
        log(LogLevel.Log, `Starting Automation Strategy for: ${strategies.bundleId}`);
        ...
        let params;
        let signingIdentity = {
            xcodeOrgId: strategies.xcodeOrgId,
            xcodeSigningId: strategies.xcodeSigningId,
            bundleId: strategies.bundleId,
            appPath: strategies.appPath,
            simulatorAppPath: strategies.simulatorAppPath
        };
